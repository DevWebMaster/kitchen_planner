...
class Xcrud
{
    ...
    public function where_sql($where_sql)
    {
        if($where_sql != '')
        {
            $this->where[] = array(
...
//            foreach ($fdata as $key => $fitem)
//            {
//                $this->group_by = 'group by[$fitem['table'] . '.' . $fitem['field']] = 'date';
//                $this->group_by[$key] = isset($fitem['value']) ? $fitem['value'] : $direction;
//            }
            ...
            foreach ($fdata as $fitem)
            {
                ...
                switch ($type)
                {
                    ...
                    default:
                        ...
                        $this->field_attr[$fitem['table'] . '.' . $fitem['field']] = $attr;
                        $this->field_cmd[$fitem['table'] . '.' . $fitem['field']] = 'date';
                        break;
                }
    ...
    public function add_command($fields = '', $command = '')
    {
        $fdata = $this->_parse_field_names($fields, 'add_command');

        foreach ($fdata as $fitem)
        {
            $this->field_cmd[$fitem['table'] . '.' . $fitem['field']] = $command;
        }
    ...
    protected function _post($field = '', $default = false, $filter = false)
    {
        ...
        {
            //if (get_magic_quotes_gpc()) //jsling get_magic_quotes_gpc alwayz zero
            if (false)
            {
    ...
    protected function _list()
    {
        ...
        $limit = $this->_build_limit($this->result_total);
        $sql = "SELECT {$select} \r\n FROM `{$this->table}`\r\n {$table_join}\r\n {$where}\r\n {$this->group_by}\r\n {$order_by}\r\n {$limit}";
//        if($this->table == "gb_parcel_order")
//        {
//            //$tempSql = "select date(created_at), count(order_id) as 'Total Orders', sum(total) as 'Total Amount' from gb_parcel_order group by date(created_at)";
//            $tempSql = "SELECT date(`gb_parcel_order`.`created_at`) AS `gb_parcel_order.created_at` 
//                        ,count(`gb_parcel_order`.`order_id`) AS `gb_parcel_order.order_id` 
//                        ,sum(`gb_parcel_order`.`total`) AS `gb_parcel_order.total` 
//                        ,(0) AS `primary_key` 
//  
//                          FROM `gb_parcel_order`
//  
//                          WHERE (`gb_parcel_order`.`customer_id`='1')
//                          
    ...
    protected function _build_select_list($csv = false)
    {
        ...
        foreach ($this->columns as $field_index => $val)
        {
            if ($val)
            {
                ...
                else if(isset($this->field_cmd[$field_index]))
                {
                    $columns[] = $this->field_cmd[$field_index]."(`{$val['table']}`.`{$val['field']}`) AS `{$val['table']}.{$val['field']}` \r\n";
                }
                else
                {
