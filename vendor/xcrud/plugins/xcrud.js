...
jQuery(document).on("ready xcrudreinit", function() {
	//var $ = jQuery;
	if (jQuery(".xcrud").size()) {
		jQuery(".xcrud").on("change", ".xcrud-actionlist", function() {
			var container = Xcrud.get_container(this);
			var data = Xcrud.list_data(container);
			Xcrud.request(container, data);
		});
		jQuery(".xcrud").on("change", ".xcrud-daterange", function() {
			var container = Xcrud.get_container(this);
			if (jQuery(this).val()) {
				jQuery(container).find(".xcrud-datepicker-from").datepicker("setDate", new Date((jQuery(this).find('option:selected').data('from')  + new Date().getTimezoneOffset() * 60) * 1000));
				jQuery(container).find(".xcrud-datepicker-to").datepicker("setDate", new Date((jQuery(this).find('option:selected').data('to') + new Date().getTimezoneOffset() * 60) * 1000));
			} else {
				jQuery(container).find(".xcrud-datepicker-from,.xcrud-datepicker-to").val('');
			}
		});
		jQuery(".xcrud").on("change", ".xcrud-columns-select", function() {
			var container = Xcrud.get_container(this);
			var type = jQuery(this).children("option:selected").data('type');
			var fieldname = jQuery(this).children("option:selected").val();
			Xcrud.change_filter(type, container, fieldname);
		});
		jQuery(".xcrud").on("click", ".xcrud-action", function() {
			var confirm_text = jQuery(this).data('confirm');
			if (confirm_text && !window.confirm(confirm_text)) {
				return;
			} else {
				var container = Xcrud.get_container(this);
				var data = Xcrud.list_data(container, this);
				if (jQuery(this).hasClass('xcrud-in-new-window')) {
					Xcrud.new_window_request(container, data);
				} else {
					if (data.task == 'save') {
						if (!Xcrud.validation_error) {
		...
		jQuery(".xcrud").on("click", ".xcrud-toggle-show", function() {
			var container = jQuery(this).closest(".xcrud").find(".xcrud-container:first");
			var closed = jQuery(this).hasClass("xcrud-toggle-down");
			if (closed) {
				jQuery(container).stop(true, true).delay(100).slideDown(200, function() {
					jQuery(document).trigger("xcrudslidedown");
					jQuery(container).trigger("xcrudslidedown");
				});
				
				jQuery(this).closest(".xcrud").find(".xcrud-main-tab").slideUp(200);
			} else {
				jQuery(container).stop(true, true).slideUp(200, function() {
					jQuery(document).trigger("xcrudslideup");
					jQuery(container).trigger("xcrudslideup");
				});
				jQuery(this).closest(".xcrud").find(".xcrud-main-tab").delay(100).slideDown(200);
			}
			return false;
		});
		jQuery(".xcrud").on("keypress", ".xcrud-input", function(e) {
			return Xcrud.pattern_callback(e, this);
		});
		jQuery(".xcrud").on("click", ".xcrud-search-toggle", function() {
			jQuery(this).hide(200);
			jQuery(this).closest(".xcrud-ajax").find(".xcrud-search").show(200);
			return false;
		});
		jQuery(".xcrud").on("keydown", ".xcrud-searchdata", function(e) {
			if (e.which == 13) {
				var container = Xcrud.get_container(this);
				var data = Xcrud.list_data(container);
				data.search = 1;
		...
		jQuery(".xcrud").on("change", ".xcrud-upload", function() {
			var container = Xcrud.get_container(this);
			var data = Xcrud.list_data(container);
			Xcrud.upload_file(this, data, container);
			return false;
		});
		jQuery(".xcrud").on("click", ".xcrud-remove-file", function() {
			var container = Xcrud.get_container(this);
			var data = Xcrud.list_data(container);
			Xcrud.remove_file(this, data, container);
			return false;
		});
		jQuery(".xcrud").on("click", ".xcrud_modal", function() {
			var content = jQuery(this).data("content");
			var header = jQuery(this).data("header");
			Xcrud.modal(header, content);
			return false;
		});
		jQuery(".xcrud-ajax").each(function() {
		//	Xcrud.init_datepicker(this);
		//	Xcrud.init_datepicker_range(jQuery(this).find('.xcrud-columns-select option:selected').data('type'), this);
			Xcrud.depend_init(this);
			Xcrud.map_init(this);
			Xcrud.check_fixed_buttons();
			Xcrud.init_tooltips(this);
