	...
	function contactForm()
{
 ...
 //    }
	'use strict';
	$(function() {
	    var $radios = $('input:radio[name=dzRole]');
	    if($radios.is(':checked') === false) {
	        $radios.filter('[value=customer]').prop('checked', true);
	    }
	});
	var msgDiv;
	...
	$(".dzLoginForm").on('submit', function(e)
	{
		e.preventDefault();	//STOP default action
		$('.dzFormMsg').html('<div class="gen alert alert-success">Submitting..</div>');
		var dzFormAction = $(this).attr('action');
			...
			success: function(dzRes){
                ...
                if(dzRes.type == 'login' && dzRes.status == 1){
                	// setInterval(function(){
						...
						window.location = href.toString().replace('login', 'index');
                	// }, 2000)
                }
			}
		})
	});
	$(".dzUpdateForm").on('submit', function(e)
	{
		e.preventDefault();	//STOP default action
		$('.dzFormMsg').html('<div class="gen alert alert-success">Submitting..</div>');
		var dzFormAction = $(this).attr('action');
		...
		$.ajax({
			...
			success: function(dzRes){
                ...
                if(dzRes.type == 'update' && dzRes.status == 1){
                	setInterval(function(){
                		var href = window.location;
						window.location = href.toString().replace('account', 'index');
                	}, 2000)
                }
			}
		})
