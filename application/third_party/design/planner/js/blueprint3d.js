            ...
            Dimensioning.cmToMeasure = function (cm) {
                switch (Core.Configuration.getStringValue(Core.configDimUnit)) {
                    case Core.dimInch:
                        ...
                        // var len = inches*25.4 + feet*304.8;
                        return feet + "'" + inches + '"';
...
(function (BP3D) {
    ...
    (function (Model) {
        ...
        var defaultRoomTexture = {
            url: "planner/rooms/textures/hardwood.png",
            scale: 400
        };
        /**
...
(function (BP3D) {
    ...
    (function (Floorplanner) {
        ...
        var wallColorHover = "#ffa200";
        var edgeColor = "#888888";
        var edgeColorHover = "#ffa200";
        var edgeWidth = 1;
        ...
        var FloorplannerView = (function () {
            ...
            FloorplannerView.prototype.drawAngleLabel = function (edge) {
                if(edge.corners != null){
                    var pos = edge.corners;
                    for(var i = 0; i < pos.length; i++) {
                        if(i == 0){
                            var dx1=pos[i+1].x - pos[i].x;
                            var dy1=pos[i+1].y - pos[i].y;
                            var dx2=pos[pos.length-1].x - pos[i].x;
                            var dy2=pos[pos.length-1].y - pos[i].y;
                            var angle1=Math.atan2(dy1,dx1);
                            var angle2=Math.atan2(dy2,dx2);
                        }else if(i == pos.length-1){
                            var dx1=pos[0].x - pos[i].x;
                            var dy1=pos[0].y - pos[i].y;
                            var dx2=pos[i-1].x - pos[i].x;
                            var dy2=pos[i-1].y - pos[i].y;
                            var angle1=Math.atan2(dy1,dx1);
                            var angle2=Math.atan2(dy2,dx2);
                        }else {
                            var dx1=pos[i+1].x - pos[i].x;
                        ...
                        this.context.moveTo(this.viewmodel.convertX(pos[i].x), this.viewmodel.convertY(pos[i].y));
                        this.context.arc(this.viewmodel.convertX(pos[i].x), this.viewmodel.convertY(pos[i].y), 20, angle1, angle2, false);
                        this.context.closePath();
                        this.context.fillStyle=cornerColorHover;
                        // this.context.globalAlpha=0.25;
                        this.context.fill();

                        var angle=parseInt((angle2-angle1)*180/Math.PI+360)%360 + "\"";
                        this.context.font = "normal 12px Arial";
                        this.context.fillStyle = "#000000";
                        this.context.textBaseline = "middle";
                        this.context.textAlign = "center";
                        this.context.strokeStyle = "#ffffff";
                        this.context.lineWidth = 4;
                        this.context.strokeText(angle, this.viewmodel.convertX(pos[i].x-35), this.viewmodel.convertY(pos[i].y-15));
                        this.context.fillText(angle, this.viewmodel.convertX(pos[i].x-35), this.viewmodel.convertY(pos[i].y-15));
                    }
                }
            }
            /** */
...
(function (BP3D) {
    ...
    (function (Three) {
        Three.Edge = function (scene, edge, controls) {
            ...
            var texture = null;
            var lightMap = THREE.ImageUtils.loadTexture("planner/rooms/textures/walllightmap.png");
    ...
    (function (Three) {
        Three.Main = function (model, element, canvasElement, opts) {
            ...
            this.centerCamera = function () {
                ...
                var distance = model.floorplan.getSize().z * 1.3;
