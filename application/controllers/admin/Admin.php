...
class Admin extends MY_Controller {
	...
	public function indexx(){
		$this->load->library('xxxcrud');
                $this->xxxcrud->table('module_access');
		$data['xxxcrud']=$this->xxxcrud->render();
		$this->load->view('admin/includes/_header');
	...
	public function datatable_json(){				   					   
		$records['data'] = $this->admin_model->get_all_admins();
		...
		{  
			...
			$data[]= array(
				...
				'<a title="View" class="view btn btn-sm btn-info" href="'.base_url('admin/admin/edit/'.$row['id']).'"> <i class="fa fa-eye"></i></a>
				<a title="Edit" class="update btn btn-sm btn-warning" href="'.base_url('admin/admin/edit/'.$row['id']).'"> <i class="fa fa-pencil-square-o"></i></a>
				<a title="Delete" class="delete btn btn-sm btn-danger" href='.base_url("admin/admin/delete/".$row['id']).' title="Delete" onclick="return confirm(\'Do you want to delete ?\')"> <i class="fa fa-trash-o"></i></a>'
			);
		}
		$records['data']=$data;
	...
	public function add(){
		...
		$this->rbac->check_operation_access(9002); // check opration permission
	...
	public function edit($id = 0){
		...
		$this->rbac->check_operation_access(9003); // check opration permission
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
					...
					$this->session->set_flashdata('errors', $data['errors']);
					redirect(base_url('admin/admin/admin_edit/'.$id),'refresh');
			}
			else{
				$data = array(
					'username' => $this->input->post('username'),
