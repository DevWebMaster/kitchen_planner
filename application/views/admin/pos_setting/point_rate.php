  ...
  $(document).ready(function() {
    ...
    $('#uploadForm').submit(function(e) {
      ...
      if($('#min_point').val() && $('#max_point').val() && $('#price').val()){
        ...
        $.ajax({
          url: '<?= site_url(); ?>admin/pos_setting/save_point_rate',
          type: 'POST',
          data: formData,       
          cache: false,
          contentType: false,
          ...
          success: function(response) {
            ...
            if(!result.status){
              ...
              var selected_pos1 = $('#pos_id').val();
              init_point_rate_list(selected_pos1);
              $('#min_point').val('');
              $('#max_point').val('');
              $('#price').val('');
            }
    ...
    $('#pos_id').change(function(){
      var selected_pos = $('#pos_id').val();
      init_point_rate_list(selected_pos);
    });

    $('#point_rate_list tbody').on('click', 'td a.delete-row', function(){
      var id = $(this).attr('id');  
      $.ajax({
        url: '<?= site_url(); ?>admin/pos_setting/delete_point_rate',
        type: 'POST',
        data: {id: id},
        success: function(response){
    ...
    $('#btn_update').click(function(){
      ...
      $.ajax({
        url: '<?= site_url(); ?>admin/pos_setting/edit_point_rate',
        type: 'POST',
    ...
    function init_point_rate_list(pos_id){
      $('#point_rate_list').DataTable({
        ...
        'serverMethod': 'post',
        'ajax': {
            'url':'<?= site_url(); ?>admin/pos_setting/get_point_rate_list',
            'data': {pos_id: pos_id},
        },
        'columns': [
           { data: 'min_point' },
